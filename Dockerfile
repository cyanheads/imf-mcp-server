...
# Install build tools required for native modules (@duckdb/node-api uses node-gyp)
...
RUN mkdir -p /var/log/imf-mcp-server && chown -R bun:bun /var/log/imf-mcp-server

# Writable data dir for the on-disk DuckDB canvas store, owned by the runtime user.
RUN mkdir -p /usr/src/app/.cache && chown -R bun:bun /usr/src/app/.cache
...
ENV LOGS_DIR="/var/log/imf-mcp-server"
